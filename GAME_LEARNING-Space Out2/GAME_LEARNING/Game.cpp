...
void GamePaint(HDC hDC)
{
	...
	// Draw the sprites
	g_pGame->DrawSprites(hDC);//无论游戏是否牌演示模式，都将绘制子画面
...
void HandleKeys()
{
	if(!g_bGameOver && !g_bDemo){
		//Move the car based upoon left/right key presses
		...
		if((++g_iFireInputDelay > 6) && GetAsyncKeyState(VK_SPACE) < 0)
		{
			...
			PlaySound((LPCWSTR)IDW_MISSILE, g_hInstance, SND_ASYNC |
...
BOOL SpriteCollision(Sprite* pSpriteHitter, Sprite* pSpriteHittee)
{
		...
		pHitter == g_pJellyBitmap || pHitter == g_pTimmyBitmap)))
	{
		...
		PlaySound((LPCWSTR)IDW_LGEXPLODE, g_hInstance, SND_ASYNC |
			SND_RESOURCE);
		...
		pHitter == g_pJMissileBitmap || pHitter == g_pTMissileBitmap)))
	{
		...
		if (--g_iNumLives == 0)
		{
			...
			PlaySound((LPCWSTR)IDW_GAMEOVER, g_hInstance, SND_ASYNC |
				SND_RESOURCE);
			g_bGameOver = TRUE;
...
void SpriteDying(Sprite* pSpriteDying)
{
		...
		pSpriteDying->GetBitmap() == g_pTMissileBitmap)
	{
		// Play the small explosion sound
		if(!g_bDemo)
			PlaySound((LPCWSTR)IDW_SMEXPLODE, g_hInstance, SND_ASYNC |
			SND_RESOURCE | SND_NOSTOP);
...
BOOL WriteHiScores(){
	...
	for(int i = 0; i < 5; ++i){
		TCHAR cData[6];
		wsprintf(cData,TEXT("%05d"),g_iHiScores[i]);
		DWORD DWBytsWritten;
		if(!WriteFile(hFile,&cData,5,&DWBytsWritten,NULL)){
			CloseHandle(hFile);
			return FALSE;
		}
	}
