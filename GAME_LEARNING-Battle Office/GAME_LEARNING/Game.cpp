...
void GamePaint(HDC hDC)
{
  ...
  wsprintf(szText, TEXT("%d"), g_iHits);
    ...
    for (int i = 0; i < 5; i++)
      ...
      else
      {
        if (--g_iGuyDelay[i] == 0)
        {
          ...
          PlaySound((LPCWSTR)IDR_TAUNT, g_hInstance, SND_ASYNC | SND_RESOURCE);
          ...
          if (++g_iMisses == 5)
          {
           ...
           PlaySound((LPCWSTR)IDR_BOO, g_hInstance, SND_ASYNC | SND_RESOURCE);
...
void MouseButtonDown(int x, int y, BOOL bLeft)
{
  ...
  if (!g_bGameOver && bLeft)
  {
    ...
    if ((pSprite = g_pGame->IsPointInSprite(x, y)) != NULL)
    {
      ...
      PlaySound((LPCWSTR)IDR_WHACK, g_hInstance, SND_ASYNC | SND_RESOURCE);
